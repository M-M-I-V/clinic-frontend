...
import { createMedicalVisit } from "@/lib/api"
...
export function MedicalVisitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createMedicalVisit(formData)
      setSuccess(true)
      ...
      setError(err instanceof Error ? err.message : "Failed to create medical visit")
    } finally {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <h3 className="text-lg font-semibold text-foreground">Medical visit created successfully!</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex gap-4 justify-end">
        ...
        >
          {isSubmitting ? "Creating..." : "Create Medical Visit"}
        </Button>
      </div>
    </form>
  )
