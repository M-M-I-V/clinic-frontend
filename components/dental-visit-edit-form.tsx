...
import { updateDentalVisit, getDentalVisit, usePatientsList } from "@/lib/api"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function DentalVisitEditForm({ visitId }: DentalVisitEditFormProps) {
  ...
  const [pendingFormData, setPendingFormData] = useState<DentalVisitFormData | null>(null)

  const [formData, setFormData] = useState<DentalVisitFormData>({
    patientId: "",
    visitDate: "",
    chiefComplaint: "",
    temperature: "",
  ...
  const handleConfirmUpdate = async () => {
    ...
    try {
      ...
      await updateDentalVisit(visitId, pendingFormData)
      setSuccess(true)

      setTimeout(() => {
        router.push(`/visits/${visitId}?type=DENTAL`)
      }, 2000)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to update dental visit")
    } finally {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <h3 className="text-lg font-semibold text-foreground">Dental visit updated successfully!</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex gap-4 justify-end">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(`/visits/${visitId}?type=DENTAL`)}
          ...
          >
            {isSubmitting ? "Updating..." : "Update Dental Visit"}
          </Button>
        </div>
      </form>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to update this dental visit? Please review your changes before confirming.
            </AlertDialogDescription>
          </AlertDialogHeader>
